...
if len(sys.argv) == 2:
    ...
    if (rank == 0):
        ...
        #xlabels = np.arange(minVal, round(maxVal, 2), round(step,2))
        ylabels = np.flipud(xlabels)
        #xlabels = np.arange(start=round(minVal, 2))
        #ylabels = np.linspace(start=minVal, stop=round(maxVal, 2))
        print(xlabels)
        print(len(xlabels))
        ax = sns.heatmap(heat_grid, square=True, cmap="BuPu")
        print("ax,slim", ax.get_xlim())
